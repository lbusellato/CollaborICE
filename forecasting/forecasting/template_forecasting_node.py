...
class ForecastingNode(LifecycleNode):
    def __init__(self, name: str = "forecasting", target_hz: int = 30, publish_topic_name: str | None= None, *args, **kwargs):
        ...
        logger.info(f"ForecastingNode initialized with target_hz={self.target_hz}. Starting to listen to {LEAP_TOPIC_NAME} and publishing to {publish_topic_name}.")
    ...
    def forecast(self):
        with self.lock:
            ...
            out_data = {
                # "TEST": 0.0,
                #"timestamp": time.time(),
                "timestamp": timestamp, # Better ROS practice than time.time()
                "future_position": [],
                "future_trajectory": [],
                "future_rotation": [],
                "future_rotations": [],
