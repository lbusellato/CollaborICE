...
try:
    ...
    from ..forecasting_utils import (
        ...
        uncertainty_covariance_det,
        uncertainty_covariance_trace,
    )
...
def get_model_path():
    try:
        pkg_dir = get_package_share_directory("forecasting")
    except PackageNotFoundError:
        pkg_dir = os.path.join(os.getcwd(), "src", "forecasting")
    ckpt = os.path.join(
        pkg_dir,
        "checkpoints",
        "ProbV2-transofrmer_with_rotation_1000_r2-hz=30-Tin=30-Tout=30.ckpt",
    )
...
class NNNode(ForecastingNode):
    ...
    def __init__(self, model, name: str = "nn_forecasting_node", target_hz: int = 30, publish_topic_name: str|None = None, **kwargs):
        ...
        # Choose uncertainty function
        if isinstance(self.model, RNNWRotation):
            self.uncertainty_function = uncertainty_heuristic
        elif isinstance(self.model, ProbV2):
            self.uncertainty_function = uncertainty_covariance_det
        else:
            raise ValueError("Unsupported model type for uncertainty calculation: " f"{type(self.model)}")
    ...
    def do_forecasting(self, input_trajectory_raw: torch.Tensor) -> dict:
        ...
        # Run model inference
        with torch.no_grad():
            pred = self.model(traj_gpu, rot_gpu)

        # Parse outputs
        trajectory, rotation, uncertainty = self.parse_output(pred, first)

        tmp = {
            "future_position": trajectory[-1].tolist(),
            "future_trajectory": trajectory.tolist(),
            "future_rotation": rotation.tolist(),
            "future_rotations": rotation.tolist(),
